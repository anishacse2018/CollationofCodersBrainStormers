...
function EEE(){
   ...
   var res= await axios.get(`https://competitive-coding-api.herokuapp.com/api/leetcode/${codechefid}`
	...
	for(var i=0;i<n;i++){
 if('EEE'==snapshot._delegate.docs[i]._document.data.value.mapValue.fields.deptartment.stringValue){
...
total.sort((a, b) => {
    ...
    return(
        <>
{!dis && (<button style={{backgroundColor:"white",color:'deeppink',fontWeight:"bold"}} onClick={display}>CLICK TO VIEW EEE LEADERBOARD</button>)}   
  {dis && (<>
  <div style={{marginRight:"600px"}}>
  <h1 style={{color:"white",fontFamily:"Apple Chancery, cursive",fontSize:"30px",marginLeft:"290px"}} >EEE LEADERBOARD</h1>
